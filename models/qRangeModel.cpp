...
// try to comment above and uncommend below if it works because below code is modern one.
/*QRangeModelPrivate::QRangeModelPrivate(QRangeModel *qq)
    : q_ptr(qq)
{
    init();
...
*/
/*!
  Compatible with Qt4. since you’re likely using QScopedPointer (or std::unique_ptr) for d_ptr,
 you don’t need to manually delete it — the smart pointer will handle that automatically.
 Why:
In your Qt 4 code, d_ptr was probably a raw pointer, so manual deletion was necessary.
 In Qt 6, QScopedPointer<QRangeModelPrivate> will delete its owned object when QRangeModel is destroyed,
 so doing delete d_ptr; is both unnecessary and unsafe.
*/
